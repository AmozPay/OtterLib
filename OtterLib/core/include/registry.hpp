...
public:
  ...
  template <class C>
  sparse_array<C> &register_component(){
    auto res = _type_map.try_emplace(std::type_index(typeid(C), std::make_any<sparse_array<C>>()));
    if (res.second == true)
  ...
  template<class C>
  sparse_array<C> &get_components() {
    return std::any_cast<C>(*_type_map.find(std::type_index(typeid(C))));
  }

  template<class C>
  sparse_array<C> const &get_components() const {
    return std::any_cast<C>(*_type_map.find(std::type_index(typeid(C))));
  }
