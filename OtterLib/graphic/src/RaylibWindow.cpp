...
namespace Otter::Graphic::Raylib
{
    ...
    void RaylibWindow::setFramerateLimit(const int &framerate) {
        SetTargetFPS(framerate);
    }

    void RaylibWindow::setScreenHeight(const int &height) {
        SetWindowSize(GetScreenWidth(), height);
    }

    void RaylibWindow::setScreenWidth(const int &width) {
        SetWindowSize(width, GetScreenHeight());
    }

    void RaylibWindow::setScreenSize(const int &width, const int &height) {
    ...
    void RaylibWindow::startDrawing() {
        BeginDrawing();
    }
