...
{
    boost::with_lock_guard(*this->_data_lock, [this, &data] {
        data.write(_data.str().c_str(), 22);
        _data.str("");
    });
}

void Otter::Network::Session::send(const std::string& data)
{
    _socket.send(this->_endpoint, data);
}

const Otter::Network::udp::endpoint& Otter::Network::Session::get_endpoint(void) const
{
    return _endpoint;
}