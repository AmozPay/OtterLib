...
Otter::Network::Socket::Socket(std::uint16_t port) :
...
void Otter::Network::Socket::_recv(void)
{
    ...
    _socket.async_receive_from(boost::asio::buffer(*data, 1024), *source,
        [this, source, data] (const boost::system::error_code& err, std::size_t rcvd) {
            if (err) {
                ...
                if (con == this->_sessions.end()) {
                    con = boost::with_lock_guard(_connections_lock, [this, source] {
                            return this->_sessions.insert({*source, Session(*source, *this)}).first;
                        });
                }
                std::cout << "Received " << rcvd << " byte(s) from " << *source << std::endl;
                boost::with_lock_guard(*con->second._data_lock, [con, data, rcvd] {
                    con->second._data.write((*data).data(), rcvd);
                });
            }
            this->_recv();
        });
